<?php
/**
 * Defines the Text_LiteralLexer class.
 *
 * @package doc
 * @author Rob Tuley
 * @version SVN: $Id$
...
/**
 * Visitor that parses out sections of literal text.
 *
 * @package doc
 */
class Text_LiteralLexer extends T_Text_LexerTemplate
{

    /**
     * Parses out literal code.
     *
    ...
    protected function parse(T_Text_Parseable $element)
    {
        ...
        $regex = '/'.preg_quote('~~~~').'(.+?)'.preg_quote('~~~~').'/su';
        ...
        for ($i=0; $i<$num; $i++) {
            ...
            /* php */
            $emph = new Text_Literal($matches[1][$i][0]);
